...
OpenCV-based real-time visualization that draws:
  • Bin boundary polygons (color-coded)
...
from bin_tracker.engine.bin_assignment import BinEvent
...
class OverlayUI:
    ...
    def _draw_bins(self, img: np.ndarray, active_ids: set[int]) -> None:
        overlay = img.copy()

        for region in self._bins:
            color = _BIN_COLORS[region.bin_id % len(_BIN_COLORS)]
